...
// prettier-ignore
let tierFilter = "all", tagFilter = "", query = "", sortBy = "id";
...
function applyFilter() {
  filtered = sortProblems(
    filterProblems(all, { tierFilter, tagFilter, query }),
    sortBy,
  );
  page = 1;
  render();
  ...
  const rows = items
    ...
    <tr class="problem-row" data-id="${escHtml(p.id)}">
      ...
      <td class="tag-list">${(p.tags ?? [])
        ...
        .map(
          (t) =>
            `<span class="tag"${TAG_NAMES_KO[t] ? ` title="${escHtml(TAG_NAMES_KO[t])}"` : ""}>${escHtml(t)}</span>`,
        )
        .join("")}</td>
...
function renderModal(p) {
  ...
  const tags = (p.tags ?? [])
    .map(
      (t) =>
        `<span class="modal-tag" data-tag="${escHtml(t)}">${escHtml(t)}${TAG_NAMES_KO[t] ? ` (${escHtml(TAG_NAMES_KO[t])})` : ""}</span>`,
    )
    .join("");
...
fetch("index.json")
  ...
  .then((data) => {
    ...
    if (tagParam && topTags.includes(tagParam)) {
      tagFilter = tagParam;
      sel.value = tagParam;
    }
    if (sortParam === "level") {
