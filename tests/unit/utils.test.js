...
describe("sortProblems", () => {
  ...
  it("기본값(id)은 번호 오름차순 정렬", () => {
    expect(sortProblems(problems).map((p) => p.id)).toEqual([10, 20, 30]);
  });

  it("level 정렬은 난이도 오름차순, 동률이면 번호순", () => {
    expect(sortProblems(problems, "level").map((p) => p.id)).toEqual([
      20, 30, 10,
    ]);
  });

  it("level 누락 문제는 0으로 취급", () => {
    const withMissing = [{ id: 2, level: 1 }, { id: 1 }];
    expect(sortProblems(withMissing, "level").map((p) => p.id)).toEqual([
      1, 2,
    ]);
  });
